...
def write_to_file(buff, filename):
    wf = wave.open(filename, 'wb')
    wf.setnchannels(CHANNELS)
    wf.setsampwidth(p.get_sample_size(FORMAT))
    wf.setframerate(SAMPLE_RATE)
    wf.writeframes(b''.join(buff.tostring()))
    wf.close()
...
def band_pass_filter(buff, low_freq, high_freq, plot=False):
    ...
    def objective(x):
        if x > low_index and x < high_index:
            return True
        if x > num_samples - high_index and x < num_samples - low_index:
            return True
        return False
...
def mask_filter(buff, segments, plot=False):
    if plot:
        plt.figure()
        plt.plot(buff)
